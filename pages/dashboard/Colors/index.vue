      ...
      >
        <template v-slot:item.Deleted="{ item }">
         ...
         <template v-slot:item.Edit="{ item }">
           <a :href="`/dashboard/Colors/Edit/${item.id}`">
...
export default {
  ...
  methods:{
    ...
    deleted(i){
      let that=this;
      this.$axios.delete(this.$url+'v1/dashboard/colors/'+i,
        {
         headers:{
    ...
    loaditem(){
      let that=this;
      this.$axios.get(this.$url+'v1/dashboard/colors',
        {
          params: {
            limit:100,
            page:this.page
